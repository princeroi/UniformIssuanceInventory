...
async function createUser() {
    ...
    try {
        ...
        if (result.success) {
            ...
            const passwordMatch = result.message.match(/Generated password: (.+?) \(/);
...
async function updateUser() {
    ...
    // Validate
    if (!firstName || !lastName || !username || !email || !password || !roleId || !departmentId || !status) {
        NotificationUI.showError('Please fill in all required fields', 'All fields marked with * are required.');
        return;
    }
...
async function toggleStatus(userId, currentStatus) {
    ...
    const confirmed = await NotificationUI.showConfirm({
        ...
        detail: newStatus === 'Inactive' ? 'The user will not be able to log in.' : 'The user will be able to log in again.',
...
async function deleteEmployee(userId, userName) {
    const confirmed = await NotificationUI.showConfirm({
        ...
        detail: 'This action cannot be undone. All user data will be permanently removed.',
        type: 'danger',
        confirmText: 'Delete'
    });
